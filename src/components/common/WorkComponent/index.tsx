...
export default function WorkComponent({ company, date, position, text }: WorkComponentProps) {
  ...
  const isMoment = date.split(' ')[date.split(' ').length-1] === 'momento' ? true : false
  ...
  return (
    <li className="relative max-w-[1000px] flex bg-backgroundSecondary flex-col p-4 rounded-md m-auto items-start justify-start gap-4 text-sm lg:text-lg md:flex-row md:w-full md:bg-transparent md:justify-between">
      ...
      <div className="absolute md:hidden lg:block md:relative top-2 right-2 border-dashed p-1 border-2 border- border-black rounded-full">
        <span className={`h-2 w-2 md:h-5 md:w-5 rounded-full flex items-center justify-center text-xl font-bold text-background ${isMoment ? 'bg-gradientStart' : 'bg-black'}`}></span>
      </div>
